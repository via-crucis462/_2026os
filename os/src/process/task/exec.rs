...
use crate::mm::{translated_write, MemorySet, KERNEL_SPACE};
use crate::process::registry::{tid2task, TID2TCB};
use crate::process::scheduler::runqueue::wake_up_task;
use crate::process::signal::{inner_has_pending_sigkill, SigHand, Signal, SignalFlags, Sigpending};
...
fn parse_shebang(data: &[u8]) -> Result<Option<(String, Option<String>)>, isize> {
    ...
    let Some(line_end) = data[2..search_end]
        .iter()
...
impl TaskStruct {
    pub fn do_exec(
        ...
        if !path_exists {
            envs.push(
                "PATH=/bin:/sbin:/usr/bin:/usr/sbin:/musl:/musl/ltp/testcases/bin".to_string(),
            );
        ...
        let mut executable_file: Arc<dyn File + Send + Sync> = app_inode.clone();
        {
            ...
            if is_dir || !can_exec {
                warn!(
                    "[kernel] sys_exec: target '{}' is not executable (is_dir={}, mode={:#o})",
                    path, is_dir, stat.mode
                );
        ...
        debug!(
            "[kernel] sys_exec: after open_file, size={}",
            app_inode.inode().get_size()
        );
        ...
        if shebang.is_some() || app_name.ends_with(".sh") {
            let (interpreter, optional_arg) =
                shebang.unwrap_or_else(|| ("/musl/busybox".to_string(), Some("sh".to_string())));
            info!(
                "[kernel] sys_exec: script '{}' requests interpreter '{}' with option {:?}",
                app_name, interpreter, optional_arg
            );
            let Some(inode) = open_file(cwd.clone(), interpreter.as_str(), OpenFlags::RDONLY, 0)
            else {
                warn!(
                    "[kernel] sys_exec: failed to open script interpreter '{}'",
                    interpreter
                );
            ...
            if is_dir || !inode.get_perm().can_execute(uid, gid) {
                warn!(
                    "[kernel] sys_exec: script interpreter '{}' is not executable",
                    interpreter
                );
                return Errno::EACCES.as_isize();
            }
            ...
            if interpreter_prefix.len() < 4 || &interpreter_prefix[0..4] != b"\x7fELF" {
                warn!(
                    "[kernel] sys_exec: script interpreter '{}' is not an ELF executable",
                    interpreter
                );
    ...
    fn install_exec_image(
        ...
        let metadata_size =
            (auxv.len() * 2 + envp_ptrs.len() + 1 + argv_ptrs.len() + 1 + 1) * word_size;
        ...
        };
        {
            ...
            let mut cloexec_files = Vec::new();
            for fd in 0..files.fds.len() {
                if files.fds[fd].flags.contains(FdFlags::CLOEXEC) {
                    if let Some(file) = files.fds[fd].file.take() {
                        cloexec_files.push(file);
                    }
                    files.clear_fd(fd);
                }
            }
