...
pub fn syscall(syscall_id: usize, args: [usize; 6]) -> isize {
    ...
    trace!("[K] hart[{}] PID{} TID{} finished syscall {} with return value {:x}", get_hart_id(), current_task().unwrap().getpid(), current_task().unwrap().gettid(), syscall_id, ret);
    info!("[K] hart[{}] PID{} TID{} finished syscall {} with return value {:x}", get_hart_id(), current_task().unwrap().getpid(), current_task().unwrap().gettid(), syscall_id, ret);
