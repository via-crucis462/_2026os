...
use crate::process::{
    block_current_and_run_next_if_task, wait4_block_current, waitid_block_current,
};
...
use crate::mm::{
    prepare_user_read, prepare_user_write, translated_read, try_translated_read,
    try_translated_str, try_translated_str_with_limit, try_translated_write, UserCStringError,
};
use crate::net::socket::TcpSocket;
use crate::net::socket::UdpSocket;
use crate::process::scheduler::futex::{
    block_current_on_futex_if, remove_futex_waiter, requeue_futex_waiters,
    retire_futex_wait_queue_by_key_if_unused, retire_futex_wait_queue_if_unused,
    wake_futex_waiters, FUTEX_WAIT_QUEUES,
};
use crate::process::scheduler::runqueue::{
    SCHED_BATCH, SCHED_FIFO, SCHED_IDLE, SCHED_OTHER, SCHED_RR,
};
...
use crate::{get_hart_id, PAGE_SIZE, USER_APP_MAX_SIZE};
...
pub(crate) fn clear_child_tid_and_wake(mm: &MemorySet, clear_child_tid: usize) {
    if clear_child_tid == 0 {
        return;
    }

    // Clear first. This may populate a lazy anonymous page or resolve COW, but
    // the futex key itself is virtual for private mappings and remains stable.
    if !try_translated_write(mm, clear_child_tid as *mut u32, 0u32) {
        error!(
    ...
    // Linux's clear_child_tid path issues FUTEX_WAKE without PRIVATE_FLAG.
    // Keep the same key domain as a user FUTEX_WAIT with no private flag.
    let Some(key) = mm.futex_key(VirtAddr::from(clear_child_tid), false) else {
        return;
    };
    let queue = {
    ...
    if let Some(queue) = queue {
        ...
        retire_futex_wait_queue_if_unused(key, &queue);
    }
}

pub fn sys_sigaltstack(new_stack: *const SignalAltStack, old_stack: *mut SignalAltStack) -> isize {
    ...
    let requested = if new_stack.is_null() {
        ...
        let Some(stack) = try_translated_read(&mm, new_stack) else {
    ...
    if !old_stack.is_null()
        && !try_translated_write(&mm, old_stack, current.status_for_sp(current_sp))
    {
        return EFAULT.as_isize();
    }
...
pub use crate::process::timer::{
    add_posix_timer, delete_posix_timer, get_posix_timer_spec, remove_posix_timer, set_posix_timer,
    ITimerSpec, KernelSigEvent, PosixTimer,
};
pub use crate::{
    fs::*,
    mm::{
        mmap, translated_byte_buffer, translated_byte_buffer_mut, translated_str,
        translated_user_buffer, translated_write, MemorySet, PageTable, UserBuffer, VirtAddr,
    },
    process::{
        registry::*,
        task::{
            add_task, current_task, current_user_mm, exit_current_and_run_next,
...
use crate::syscall::epoll::{EpollEvent, EpollFile, EventFile};
#[repr(C)]
#[derive(Clone, Copy)]
pub struct Termios {
    pub c_iflag: u32,
...
pub fn sys_ppoll(
    ...
    debug!(
        "[kernel] sys_ppoll: ufds=0x{:x}, nfds={}, tmo_p=0x{:x}",
        ufds_ptr, nfds, tmo_p
    );
...
pub fn sys_exit_group(exit_code: i32) -> ! {
    ...
    info!(
        "[EXIT_GROUP] PID {} tasks cleanup done. Calling exit_current_and_run_next...",
        pid
    );
    // 正常的退出流程
    crate::timer::TIMER_MANAGER.lock().cancel_alarm(pid);
...
pub fn sys_chroot(path: usize) -> isize {
    ...
    let mm = current_user_mm();
    let path_str = {
        if let Some(s) = crate::mm::try_translated_str(&mm, path as *const u8) {
            s
        } else {
            return EFAULT.as_isize();
        }
...
pub fn sys_getresuid(ruid_ptr: *mut u32, euid_ptr: *mut u32, suid_ptr: *mut u32) -> isize {
    ...
    if ruid_ptr as usize != 0 {
        translated_write(&mm, ruid_ptr, root_uid);
    }
    if euid_ptr as usize != 0 {
        translated_write(&mm, euid_ptr, root_uid);
    }
    if suid_ptr as usize != 0 {
        translated_write(&mm, suid_ptr, root_uid);
    }
    0
}
...
fn current_wallclock_ns() -> i64 {
    crate::timer::current_realtime_ns()
}
...
pub fn sys_clock_gettime(clock_id: usize, tp: *mut TimeSpec) -> isize {
    ...
    let mm = current_user_mm();
    let mut time_spec = {
        if let Some(ts) = try_translated_read(&mm, tp) {
            ts
        } else {
            return EFAULT.as_isize();
        }
    ...
    if !try_translated_write(&mm, tp, time_spec) {
        return EFAULT.as_isize();
    }
    0
}
...
pub fn sys_ioctl(fd: usize, request: usize, argp: usize) -> isize {
    ...
    match request as u32 {
        TCGETS => {
            ...
            if argp != 0 {
                if !try_translated_write(&mm, argp as *mut Termios, termios) {
                    return EFAULT.as_isize();
                }
                0 // 成功
            } else {
        ...
        TIOCGPGRP => {
            ...
            if !try_translated_write(&mm, argp as *mut i32, pgrp) {
                return EFAULT.as_isize();
            }
            0
        }
        TIOCSPGRP => {
            ...
            let Some(new_pgrp) = try_translated_read(&mm, argp as *const i32) else {
                return EFAULT.as_isize();
            };
            if new_pgrp <= 0 {
                return EINVAL.as_isize();
        ...
        TIOCGWINSZ => {
            if !is_tty {
                warn!(
                    "[kernel] sys_ioctl: TIOCGWINSZ request on non-tty fd {}",
                    fd
                );
                return ENOTTY.as_isize();
            }
            ...
            if argp != 0 {
                if !try_translated_write(&mm, argp as *mut Winsize, winsize) {
                    return EFAULT.as_isize();
                }
                0 // 成功
            } else {
        ...
        FIONBIO => {
            let Some(nonblocking) = try_translated_read(&mm, argp as *const i32) else {
                return EFAULT.as_isize();
            };
        ...
        RTC_RD_TIME => {
            ...
            if argp != 0 {
                if !try_translated_write(&mm, argp as *mut RtcTime, rtc_time) {
                    return EFAULT.as_isize();
                }
                0
            } else {
        ...
        LOOP_SET_FD => {
            ...
            if let Some(id) = target_id {
                if crate::drivers::loopdev::LOOP_DEVICE_MANAGER
                    .set_backing_file(id, Some(backend_inode))
                {
                    0
                } else {
                    EINVAL.as_isize()
        ...
        LOOP_GET_STATUS64 => {
            ...
            if let Some(id) = target_id {
                if let Some((offset, size)) =
                    crate::drivers::loopdev::LOOP_DEVICE_MANAGER.get_info(id)
                {
                    if argp != 0 {
                        ...
                        if !try_translated_write(&mm, argp as *mut LoopInfo64, info) {
                            return EFAULT.as_isize();
                        }
                        0
                    } else {
        ...
        BLKGETSIZE64 => {
            ...
            if let Some(id) = target_id {
                if let Some((_, size)) = crate::drivers::loopdev::LOOP_DEVICE_MANAGER.get_info(id) {
                    if argp != 0 {
                        if !try_translated_write(&mm, argp as *mut u64, size as u64) {
                            return EFAULT.as_isize();
                        }
                        0
                    } else {
        ...
        LOOP_SET_STATUS64 | LOOP_SET_STATUS => 0,
        LOOP_SET_BLOCK_SIZE => {
            let bs = argp;
        ...
        LOOP_CONFIGURE => {
            let fd = {
                if let Some(f) = try_translated_read(&mm, argp as *const i32) {
                    f
                } else {
                    return EFAULT.as_isize();
                }
            ...
            let bs = {
                if let Some(b) = try_translated_read(&mm, (argp + 4) as *const u32) {
                    b as usize
                } else {
                    return EFAULT.as_isize();
                }
        ...
        0x5402 | 0x5403 | 0x5404 => {
            ...
            if let Some(t) = try_translated_read(&mm, argp as *const Termios) {
        ...
        SIOCGIFFLAGS => {
            // 获取网卡运行状态
            if let Some(mut ifr) = try_translated_read::<IfReq>(&mm, argp as *const IfReq) {
                // 网卡处于 UP (1) 且 RUNNING (0x40) 状态
                let flags: u16 = 0x1 | 0x40;
                ifr.ifru_data[0..2].copy_from_slice(&flags.to_ne_bytes());
                try_translated_write(&mm, argp as *mut IfReq, ifr);
                0
            } else {
                EFAULT.as_isize()
            }
        ...
        SIOCGIFTXQLEN => {
            if let Some(mut ifr) = try_translated_read::<IfReq>(&mm, argp as *const IfReq) {
                ...
                if try_translated_write(&mm, argp as *mut IfReq, ifr) {
                    0
                } else {
                    EFAULT.as_isize()
                }
        ...
        SIOCSIFADDR => {
            // 给 eth0 绑定新 IP！
            if let Some(ifr) = try_translated_read::<IfReq>(&mm, argp as *const IfReq) {
        ...
        0x8900..=0x89ff => {
            // 兜底：对其他未实现的 ifconfig / ip 命令配置请求返回 0
            Errno::ENOSYS.as_isize()
        }
        _ => {
            // 委托给文件自己的 ioctl（如 userfaultfd）
            let file = fd_table[fd].file.as_ref().unwrap();
            file.ioctl(request as u32, argp, &mm)
        }
    }
}
...
pub fn sys_renameat2(
    ...
    let old_path = normalize_leading_dot_path(
        if let Some(s) = try_translated_str(&mm, oldpath_ptr as *const u8) {
            s
        } else {
            return EFAULT.as_isize();
        },
    ...
    let new_path = normalize_leading_dot_path(
        if let Some(s) = try_translated_str(&mm, newpath_ptr as *const u8) {
            s
        } else {
            return EFAULT.as_isize();
        },
        ...
        let rename_locked = || -> isize {
            ...
            let moved_dentry = old_parent
                ...
                .or_else(|| {
                    old_parent.inode.find(&old_name).map(|inode| {
                        crate::fs::Dentry::new(old_name.clone(), inode, Arc::downgrade(&old_parent))
                    })
                });
            ...
            if same_dir {
                ...
                new_parent
                    .children
                    .lock()
                    .insert(new_name.clone(), moved_dentry);
            }
            0
        };
...
pub fn sys_sysinfo(sysinfo_ptr: usize) -> isize {
    ...
    if !try_translated_write(&mm, sysinfo_ptr as *mut Sysinfo, sysinfo) {
        return EFAULT.as_isize();
    }
...
pub fn sys_uname(uts: *mut UtsName) -> isize {
    let mm = current_user_mm();
    let mut uts_name = {
        if let Some(u) = try_translated_read(&mm, uts) {
            u
        } else {
            return EFAULT.as_isize();
        }
    ...
    if !try_translated_write(&mm, uts, uts_name) {
        return EFAULT.as_isize();
    }
...
pub fn sys_clone3(uargs: *const CloneArgs, size: usize) -> isize {
    ...
    let args = if size < CLONE_ARGS_SIZE_VER1 {
        match try_translated_read(&mm, uargs as *const CloneArgsV0) {
            Some(args) => CloneArgs::from(args),
            None => return EFAULT.as_isize(),
        }
    } else if size < CLONE_ARGS_SIZE_VER2 {
        match try_translated_read(&mm, uargs as *const CloneArgsV1) {
            Some(args) => CloneArgs::from(args),
            None => return EFAULT.as_isize(),
        }
    } else {
        match try_translated_read(&mm, uargs) {
            Some(args) => args,
            None => return EFAULT.as_isize(),
        }
    };
...
pub fn sys_clone(flags: usize, stack: usize, ptid: usize, arg3: usize, arg4: usize) -> isize {
    ...
    if flags & CLONE_THREAD != 0 && (flags & CLONE_SIGHAND == 0 || flags & CSIGNAL != 0) {
        return EINVAL.as_isize();
    }
...
pub fn sys_get_mempolicy(
    ...
    let mm = current_user_mm();
    if !mode.is_null() && !try_translated_write(&mm, mode, 0i32) {
        return EFAULT.as_isize();
    }
    ...
    if !nodemask.is_null() && maxnode != 0 {
        ...
        for index in 0..words {
            ...
            if !try_translated_write(&mm, unsafe { nodemask.add(index) }, value) {
                return EFAULT.as_isize();
            }
        }
    }
...
pub fn sys_exec(path: *const u8, mut args: *const usize, mut envs: *const usize) -> isize {
    ...
    if args as usize != 0 {
        loop {
            let arg_str_ptr = {
                if let Some(p) = try_translated_read(&mm, args) {
                    p
                } else {
                    return EFAULT.as_isize();
                }
    ...
    if envs as usize != 0 {
        loop {
            let env_str_ptr = {
                if let Some(p) = try_translated_read(&mm, envs) {
                    p
                } else {
                    return EFAULT.as_isize();
                }
    ...
    loop {
        ...
        if child_pid == 0 {
            ...
            if exit_code_ptr as usize != 0 {
                ...
                if !try_translated_write(&mm, exit_code_ptr, status) {
                    return EFAULT.as_isize();
                }
    ...
    loop {
        ...
        if child_pid == 0 {
            ...
            if options & WNOHANG != 0 {
                ...
                drop(proc_inner);
                if try_translated_write(&mm, infop, info) {
                    return 0;
                } else {
                    return EFAULT.as_isize();
                }
        ...
        if options & WNOWAIT == 0 {
            ...
            if child_idx.is_some() {
                if let Some(idx) = proc_inner
                    .children
                    .iter()
                    .position(|child| child.getpid() == child_pid)
                {
                    proc_inner.children.remove(idx);
                } else {
                    return ECHILD.as_isize();
...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    // 校验 tz 指针
    if _tz != 0 {
        if !prepare_user_write(&mm, _tz, 8) {
            return EFAULT.as_isize();
        }
    }
    ...
    // 校验&写入
    if !try_translated_write(&mm, ts, time_val) {
        return EFAULT.as_isize();
    }
    0
}
...
pub fn sys_utimensat(dirfd: i32, path_ptr: usize, times_ptr: usize, _flags: usize) -> isize {
    ...
    let real_time_ns = current_wallclock_ns() as usize;
    let current_sec = real_time_ns / 1_000_000_000;
    let current_nsec = real_time_ns % 1_000_000_000;
    let mut new_atime = TimeSpec {
        tv_sec: current_sec,
        tv_nsec: current_nsec,
    };
    let mut new_mtime = TimeSpec {
        tv_sec: current_sec,
        tv_nsec: current_nsec,
    };
    ...
    let (target_file, target_inode, mut old_atime, mut old_mtime, ino) = {
        if path_ptr == 0 {
            ...
            if let Some(file_obj) = &inner.fds[dirfd as usize].file {
                ...
                let ino = stat.ino;
                let old_atime = TimeSpec {
                    tv_sec: stat.atime_sec as _,
                    tv_nsec: stat.atime_nsec as _,
                };
                let old_mtime = TimeSpec {
                    tv_sec: stat.mtime_sec as _,
                    tv_nsec: stat.mtime_nsec as _,
                };
                drop(inner);
                let (old_atime, old_mtime) = if ino != 0 {
                    if let Some(&(asec, ansec, msec, mnsec)) = TIME_CACHE.lock().get(&ino) {
                        (
                            TimeSpec {
                                tv_sec: asec as usize,
                                tv_nsec: ansec as usize,
                            },
                            TimeSpec {
                                tv_sec: msec as usize,
                                tv_nsec: mnsec as usize,
                            },
                        )
                    } else {
                        (old_atime, old_mtime)
            ...
            let path_str = {
                if let Some(s) = try_translated_str(&mm, path_ptr as *const u8) {
    ...
    if times_ptr != 0 {
        let times = {
            if let Some(t) = try_translated_read(&mm, times_ptr as *const [TimeSpec; 2]) {
                t
            } else {
                return EFAULT.as_isize();
            }
    ...
    if ino != 0 {
        ...
        TIME_CACHE.lock().insert(
            ino,
            (
                new_atime.tv_sec as i64,
                new_atime.tv_nsec as i64,
                new_mtime.tv_sec as i64,
                new_mtime.tv_nsec as i64,
            ),
        );
    } else {
...
pub fn sys_nanosleep(req: *const TimeSpec, rem: *mut TimeSpec) -> isize {
    let mm = current_user_mm();
    let req_val = {
        if let Some(ts) = try_translated_read(&mm, req) {
            ts
        } else {
            return EFAULT.as_isize();
        }
...
pub fn sys_clock_nanosleep(
    ...
    let mm = current_user_mm();
    let req_val = {
        if let Some(ts) = try_translated_read(&mm, request) {
            ts
        } else {
            return EFAULT.as_isize();
        }
...
fn nanosleep_impl(req: &TimeSpec, rem: *mut TimeSpec) -> isize {
    ...
    info!(
        "[SLEEP-IN] PID {} start_ns: {}, duration_ns: {}",
    ...
    while clock_now_ns(CLOCK_MONOTONIC) < deadline_ns {
        ...
        let pending =
            (thread_pending | signal.exclusive_access().pending_flags()).bits() & !blocked.bits();
        drop(task);
        ...
        if pending != 0 {
            ...
            if rem as usize != 0 {
                let mut rem_spec = {
                    if let Some(ts) = try_translated_read(&mm, rem) {
                        ts
                    } else {
                        return EFAULT.as_isize();
                    }
                ...
                if try_translated_write(&mm, rem, rem_spec) {
                    ()
                } else {
                    return EFAULT.as_isize();
                }
...
pub fn sys_mprotect(start: usize, len: usize, prot: usize) -> isize {
    ...
    if start
        .checked_add(len)
        .map_or(true, |end| end >= USER_APP_MAX_SIZE)
    {
        return ENOMEM.as_isize();
    }
...
/// 锁定内存页，防止被换出（实际上当前内核没有做 swap 机制）
///
/// 旧实现语义为将共享映射改为私有映射，是错误的
pub fn sys_mlock(start: usize, len: usize) -> isize {
    ...
    let result = mm.mlock_range(start, len);
    match result {
        Ok(()) => 0,
        Err(errno) => errno,
    }
...
pub fn sys_brk(addr: usize) -> isize {
    ...
    trace!(
        "kernel:pid[{}] sys_brk: request addr={:#x}, current_brk={:#x}",
        task.getpid(),
        addr,
        current_brk
    );
...
pub fn sys_sigprocmask(
    ...
    let set_flags = if set_ptr.is_null() {
        None
    } else {
        let Some(set_val) = try_translated_read(&mm, set_ptr) else {
            return EFAULT.as_isize();
        };
        let mut flags = SignalFlags::from_bits_truncate(set_val as u64);
        flags.remove(SignalFlags::SIGKILL | SignalFlags::SIGSTOP);
        Some(flags)
    };

    let task = current_task().unwrap();
    let old_mask = task.inner_exclusive_access().blocked.bits() as usize;
    if !oldset_ptr.is_null() && !try_translated_write(&mm, oldset_ptr, old_mask) {
        return EFAULT.as_isize();
    }

    if let Some(set_flags) = set_flags {
...
pub fn sys_rt_sigsuspend(mask_ptr: *const usize, sigsetsize: usize) -> isize {
    ...
    let mm = current_user_mm();
    let Some(mask_bits) = try_translated_read(&mm, mask_ptr) else {
        return EFAULT.as_isize();
    };
...
pub fn sys_epoll_ctl(epfd: usize, op: i32, fd: usize, event_ptr: usize) -> isize {
    ...
    let event = if op != 2 {
        // 如果不是 EPOLL_CTL_DEL，就需要读取用户态传来的数据
        //   使用你提供的 translated_ref
        if let Some(ev) = try_translated_read(&mm, event_ptr as *const EpollEvent) {
            ev
        } else {
            return EFAULT.as_isize();
        }
    ...
    let result = match op {
        EPOLL_CTL_ADD => {
            ...
            epoll_file
                .watched_files
                .lock()
                .insert(fd, target_file_dyn.clone());
...
pub fn sys_sched_getparam(pid: isize, param_ptr: *mut SchedParam) -> isize {
    ...
    let mm = current_user_mm();
    if !try_translated_write(&mm, param_ptr, SchedParam { sched_priority }) {
        return EFAULT.as_isize();
    }
    0
}
pub fn sys_sched_setscheduler(pid: isize, policy: isize, param_ptr: *const SchedParam) -> isize {
    ...
    let mm = current_user_mm();
    let Some(param) = try_translated_read(&mm, param_ptr) else {
        return EFAULT.as_isize();
    };
...
pub fn sys_sched_setaffinity(pid: isize, cpusetsize: usize, mask_ptr: *const u8) -> isize {
    ...
    for i in 0..read_len {
        match try_translated_read::<u8>(&mm, unsafe { mask_ptr.add(i) }) {
...
pub fn sys_sched_getaffinity(pid: isize, cpusetsize: usize, mask_ptr: *mut u8) -> isize {
    ...
    for i in 0..KERNEL_CPUSET_BYTES {
        if !try_translated_write(&mm, unsafe { mask_ptr.add(i) }, bytes[i]) {
            return EFAULT.as_isize();
        }
    }
...
pub fn sys_setitimer(which: usize, new_value: usize, old_value: usize) -> isize {
    ...
    let new_timer = {
        if let Some(t) = try_translated_read(&mm, new_value as *const ITimerVal) {
            t
        } else {
            return EFAULT.as_isize();
        }
    ...
    let remain_ms = crate::timer::TIMER_MANAGER
        .lock()
        .set_alarm(pid, current_ms, delay_ms);

    if old_value != 0 {
        // 读取修改后写回
        let mut old_timer = {
            if let Some(t) = try_translated_read(&mm, old_value as *mut ITimerVal) {
                t
            } else {
                return EFAULT.as_isize();
            }
        ...
        if !try_translated_write(&mm, old_value as *mut ITimerVal, old_timer) {
            return EFAULT.as_isize();
        }
    }
...
pub fn sys_timer_create(clock_id: i32, event: *const KernelSigEvent, timer_id: *mut i32) -> isize {
    ...
    let (notify, signo, value, target_tid) = if event.is_null() {
        ...
        let Some(event) = try_translated_read(&mm, event) else {
            return EFAULT.as_isize();
        };
    ...
    if !try_translated_write(&mm, timer_id, id) {
...
pub fn sys_timer_settime(
    ...
    let Some(new_spec) = try_translated_read(&mm, new_value) else {
        return EFAULT.as_isize();
    };
    ...
    if !old_value.is_null() && !try_translated_write(&mm, old_value, previous) {
        return EFAULT.as_isize();
    }
    0
}
...
pub fn sys_ftruncate(fd: usize, len: usize) -> isize {
    ...
    if let Some(file) = &inner.fds[fd].file {
        ...
        let file_type_name: &str = {
            let a = file.as_any();
            if a.downcast_ref::<OSInode>().is_some() {
                "OSInode"
            } else if a.downcast_ref::<Stdin>().is_some() {
                "Stdin"
            } else if a.downcast_ref::<Stdout>().is_some() {
                "Stdout"
            } else if a.downcast_ref::<Stderr>().is_some() {
                "Stderr"
            } else if a.downcast_ref::<Pipe>().is_some() {
                "Pipe"
            } else if a.downcast_ref::<EpollFile>().is_some() {
                "EpollFile"
            } else if a.downcast_ref::<crate::fs::epoll::EventFile>().is_some() {
                "EventFile"
            } else if a.downcast_ref::<UserPageFaultInfo>().is_some() {
                "UserPageFaultInfo"
            } else if a.downcast_ref::<TcpSocket>().is_some() {
                "TcpSocket"
            } else if a.downcast_ref::<crate::net::socket::UdpSocket>().is_some() {
                "UdpSocket"
            } else if a.downcast_ref::<crate::net::socket::UnixSocket>().is_some() {
                "UnixSocket"
            } else if a
                .downcast_ref::<crate::syscall::bpf::BpfMapFile>()
                .is_some()
            {
                "BpfMapFile"
            } else if a
                .downcast_ref::<crate::syscall::bpf::BpfProgFile>()
                .is_some()
            {
                "BpfProgFile"
            } else {
        ...
        warn!(
            "[kernel] sys_ftruncate: fd={}, file_type={}, mode=0o{:o}",
            fd, file_type_name, typ.mode
        );
        // 鉴权
        if !file.writable() {
            return EBADF.as_isize();
...
pub fn sys_sigreturn() -> isize {
    ...
    // 验证长度一致
    assert_eq!(
        inner.trap_ctx_backup.len(),
        inner.signal_mask_backup.len(),
        "Trap context backup and signal mask backup should be in sync"
    );
    assert_eq!(
        inner.trap_ctx_backup.len(),
        inner.signal_user_context_backup.len(),
        "Trap context backup and user signal context backup should be in sync"
    );
...
pub fn sys_getrusage(who: i32, usage_ptr: *mut Rusage) -> isize {
    ...
    if try_translated_write(&mm, usage_ptr, usage) {
        0
    } else {
        EFAULT.as_isize()
    }
...
pub fn sys_rt_sigaction(
    ...
    if !old_action.is_null() {
        ...
        if !try_translated_write(&mm, old_action, prev_action) {
            return EFAULT.as_isize();
        }
    }
    ...
    // 修改tcb的信号处理行为
    let new_action = {
        if let Some(act) = try_translated_read(&mm, action) {
            act
        } else {
            return EFAULT.as_isize();
        }
    ...
    signal_hand
        .exclusive_access()
        .set_action(table_idx, new_action);
...
pub fn sys_pselect6(
    ...
    let _signal_mask_guard = if sigmask_arg as usize != 0 {
        let Some(mask_ptr) = try_translated_read(&mm, sigmask_arg) else {
            return EFAULT.as_isize();
        };
        let Some(sigsetsize) = try_translated_read(&mm, unsafe { sigmask_arg.add(1) }) else {
            return EFAULT.as_isize();
        };
        match install_temporary_signal_mask(mask_ptr, sigsetsize) {
            Ok(guard) => guard,
            Err(errno) => return errno,
        }
    } else {
    ...
    if readfds_ptr as usize != 0 {
        readfds = {
            if let Some(rf) = crate::mm::try_translated_read(&mm, readfds_ptr) {
                rf
            } else {
    ...
    if writefds_ptr as usize != 0 {
        writefds = {
            if let Some(wf) = crate::mm::try_translated_read(&mm, writefds_ptr) {
                wf
            } else {
                return crate::syscall::errno::Errno::EFAULT.as_isize();
            }
    ...
    if exceptfds_ptr as usize != 0 {
        exceptfds = {
            if let Some(ef) = crate::mm::try_translated_read(&mm, exceptfds_ptr) {
                ef
            } else {
                return crate::syscall::errno::Errno::EFAULT.as_isize();
            }
    ...
    if has_timeout {
        let timespec = {
            if let Some(ts) = try_translated_read(&mm, _timeout as *const TimeSpec) {
                ts
            } else {
                return EFAULT.as_isize();
            }
        ...
        // 1. 计算原始的毫秒数
        let mut calculated_ms = sec
            .saturating_mul(1000)
            .saturating_add(timespec.tv_nsec / 1_000_000);
    ...
    loop {
            ...
            for fd in 0..limit {
                ...
                if in_read || in_write {
                    if let Some(fd_file) = &process_inner.fds[fd].file {
                        if let Some(tcp_sock) = fd_file
                            .as_any()
                            .downcast_ref::<crate::net::socket::TcpSocket>()
                        {
                            let mut handles = alloc::vec![tcp_sock.handle];
                            if tcp_sock.is_listener.load(Ordering::SeqCst) {
                                handles.extend(tcp_sock.backlog_handles.lock().iter().copied());
                            }
                        ...
                        } else if let Some(udp_sock) = fd_file
                            .as_any()
        ...
        let mut ready_exceptfds = 0usize;
        {
            ...
            for fd in 0..limit {
                ...
                if in_read || in_write {
                    if let Some(fd_file) = &process_inner.fds[fd].file {
                        if fd_file.is_socket() {
                            if let Some(tcp_wrapper) = fd_file
                                .as_any()
                                .downcast_ref::<crate::net::socket::TcpSocket>()
                            {
                                ...
                                let socket = sockets
                                    .get_mut::<smoltcp::socket::tcp::Socket>(tcp_wrapper.handle);
                            ...
                            } else if let Some(_udp_wrapper) = fd_file
                                .as_any()
                                .downcast_ref::<crate::net::socket::UdpSocket>(
                            ) {
                                let r_status = fd_file.readable();
        ...
        if ready_count > 0 || is_timeout {
            ...
            for fd in 0..limit {
                ...
                if in_read || in_write {
                    if let Some(fd_file) = &process_inner.fds[fd].file {
                        let handles = if let Some(tcp) = fd_file
                            .as_any()
                            .downcast_ref::<crate::net::socket::TcpSocket>(
                        ) {
                            let mut handles = alloc::vec![tcp.handle];
                            if tcp.is_listener.load(Ordering::SeqCst) {
                                handles.extend(tcp.backlog_handles.lock().iter().copied());
                            }
                            handles
                        } else if let Some(udp) = fd_file
                            .as_any()
                            .downcast_ref::<crate::net::socket::UdpSocket>()
                        {
                            alloc::vec![udp.handle]
                        } else {
                            alloc::vec![]
                        };
            ...
            if ready_count > 0 {
                // 写入用户态态指针
                if readfds_ptr as usize != 0
                    && !crate::mm::try_translated_write(&mm, readfds_ptr, ready_readfds)
                {
                    return crate::syscall::errno::Errno::EFAULT.as_isize();
                }
                if writefds_ptr as usize != 0
                    && !crate::mm::try_translated_write(&mm, writefds_ptr, ready_writefds)
                {
                    return crate::syscall::errno::Errno::EFAULT.as_isize();
                }
                if exceptfds_ptr as usize != 0
                    && !crate::mm::try_translated_write(&mm, exceptfds_ptr, ready_exceptfds)
                {
                    return crate::syscall::errno::Errno::EFAULT.as_isize();
                }
                return ready_count as isize;
                ...
                // 超时返回
                if readfds_ptr as usize != 0 {
                    crate::mm::try_translated_write(&mm, readfds_ptr, 0);
                }
                if writefds_ptr as usize != 0 {
                    crate::mm::try_translated_write(&mm, writefds_ptr, 0);
                }
                if exceptfds_ptr as usize != 0 {
                    crate::mm::try_translated_write(&mm, exceptfds_ptr, 0);
                }
                return 0;
            }
        }
...
pub fn sys_times(tms_ptr: *mut usize) -> isize {
    ...
    if !try_translated_write(&mm, tms_ptr as *mut Tms, tms_val) {
        return EFAULT.as_isize();
    }
    let current_ms = get_time_ms();
    current_ms as isize
...
pub fn sys_clock_adjtime(which_clock: i32, tp: *mut Timex) -> isize {
    ...
    let mm = current_user_mm();
    let Some(mut tx) = try_translated_read(&mm, tp as *const Timex) else {
        return EFAULT.as_isize();
    };
    ...
    if !try_translated_write(&mm, tp, tx) {
        return EFAULT.as_isize();
    }
...
pub fn sys_clock_settime(which_clock: i32, tp: *const TimeSpec) -> isize {
    ...
    let mm = current_user_mm();
    let timespec = match try_translated_read(&mm, tp) {
        Some(ts) => ts,
        None => return EFAULT.as_isize(),
    };
...
pub fn sys_getrandom(buf: *mut u8, len: usize, _flags: u32) -> isize {
    let mm = current_user_mm();
    let mut user_buf = crate::mm::translated_byte_buffer_mut(&mm, buf, len);
...
pub fn sys_robust_list() -> isize {
    let task = current_task().unwrap();
    trace!(
        "kernel:pid[{}] sys_robust_list NOT IMPLEMENTED",
        task.getpid()
    );
    ENOSYS.as_isize()
}
...
pub fn sys_get_robust_list() -> isize {
    let task = current_task().unwrap();
    trace!(
        "kernel:pid[{}] sys_get_robust_list NOT IMPLEMENTED",
        task.getpid()
    );
    ENOSYS.as_isize()
}
...
pub fn sys_rt_sigtimedwait(
    ...
    let target_set_bits = {
        if let Some(b) = try_translated_read(&mm, set_ptr) {
            b
        } else {
            return EFAULT.as_isize();
        }
    ...
    if !timeout_ptr.is_null() {
        let timeout = {
            if let Some(t) = try_translated_read(&mm, timeout_ptr) {
                t
            } else {
                return EFAULT.as_isize();
            }
...
pub fn sys_prlimit64(
    ...
    info!(
        "sys_prlimit64 called with pid={}, resource={}, new_limit=0x{:x}, old_limit=0x{:x}",
        pid, resource, new_limit as u64, old_limit as u64
    );
    if pid != 0 {
    ...
    match resource {
        ...
        RLIMIT_NOFILE => {
            ...
            let old = Rlimit64 {
                cur_lmt: limit.rlim_cur,
                max_lmt: limit.rlim_max,
            };
            if !old_limit.is_null() {
                if !try_translated_write(&mm, old_limit, old) {
                    return Errno::EFAULT.as_isize();
                }
            }
            if !new_limit.is_null() {
                let new = translated_read(&mm, new_limit);
                if new.cur_lmt > new.max_lmt || new.max_lmt > signal.rlimits().nofile.rlim_max {
                    return EINVAL.as_isize();
                }
        ...
        RLIMIT_MEMLOCK => ENOSYS.as_isize(),
        RLIMIT_CORE => ENOSYS.as_isize(),
        RLIMIT_STACK => ENOSYS.as_isize(),
        RLIMIT_DATA => ENOSYS.as_isize(),
        UL_SETFSIZE => {
            ...
            if !new_limit.is_null() {
                if let Some(new) = try_translated_read(&mm, new_limit) {
        ...
        RLIMIT_DATA | RLIMIT_NPROC | RLIMIT_AS => {
            ...
            if !old_limit.is_null() {
                let old = Rlimit64 {
                    cur_lmt: target_limit.rlim_cur,
                    max_lmt: target_limit.rlim_max,
                };
                if !try_translated_write(&mm, old_limit, old) {
                    return EFAULT.as_isize();
                }
...
/// 队列键由当前 `MemorySet` 的 VMA/backing-object 元数据派生。
pub fn sys_futex(
    uaddr: *mut i32,
    op: i32,
    val: i32,
    ...
    if (uaddr as usize) & (core::mem::align_of::<i32>() - 1) != 0 {
        return EINVAL.as_isize();
    }

    let cmd = op & FUTEX_CMD_MASK;
    let private_requested = op & FUTEX_PRIVATE_FLAG != 0;
    let mm = current_user_mm();

    match cmd {
        FUTEX_WAIT | FUTEX_WAIT_BITSET => {
            if cmd == FUTEX_WAIT && (op & FUTEX_CLOCK_REALTIME) != 0 {
                return EINVAL.as_isize();
            }
            if cmd == FUTEX_WAIT_BITSET && val3 == 0 {
                return EINVAL.as_isize();
            }
            let Some(current_val) = try_translated_read(&mm, uaddr) else {
                return EFAULT.as_isize();
            };
            ...
            let deadline_us = if !timeout.is_null() {
                let Some(timeout_val) = try_translated_read(&mm, timeout) else {
                    return EFAULT.as_isize();
                };
            ...
            // Resolve the stable queue key after validating the futex word.
            let Some(key) = mm.futex_key(VirtAddr::from(uaddr as usize), private_requested) else {
                return EFAULT.as_isize();
            };
            ...
            let Some((waiter, queue)) =
                block_current_on_futex_if(key, wait_bitset, deadline_ns, || {
                    rechecked_value = try_translated_read(&mm, uaddr);
                ...
                return match rechecked_value {
                    Some(current) => {
                        warn!(
                            ...
                            current_tid, uaddr as usize, val, current,
                        );
                        EAGAIN.as_isize()
                    }
            ...
            let current = current_task().unwrap();
            let current_tid = current.gettid();
            let removed_from_queue = remove_futex_waiter(&waiter);
            let timed_out = removed_from_queue.is_some();

            if deadline_us.is_some() {
                crate::process::scheduler::nanosleep::cancel_sleep_task(current_tid);
            }

            // A timeout/signal cleanup can remove a waiter from the
            // FUTEX_REQUEUE destination rather than `queue`.  Retire that
            // actual queue too; otherwise a last requeued waiter leaves an
            // unreachable empty entry in the global key map.
            if let Some(removal) = removed_from_queue {
                if !Arc::ptr_eq(&removal.queue, &queue) {
                    retire_futex_wait_queue_if_unused(removal.key, &removal.queue);
                }
            }
            retire_futex_wait_queue_if_unused(key, &queue);
            ...
            let unmaskable =
                pending_signals.bits() & (SignalFlags::SIGKILL | SignalFlags::SIGSTOP).bits();
            warn!(
            ...
            if (pending | unmaskable) != 0 {
                warn!(
                    ...
                    current_tid, pending, unmaskable
                );
                EINTR.as_isize()
            } else if deadline_us.is_some() && timed_out {
                // Wake and CMP_REQUEUE remove their waiter before scheduling it.
                // A deadline wake leaves it queued, so cleanup above removes it.
                ETIMEDOUT.as_isize()
            } else {
                warn!(
        ...
        FUTEX_WAKE | FUTEX_WAKE_BITSET => {
            ...
            let woken = wake_futex_waiters(&queue, val as usize, wake_bitset);
            retire_futex_wait_queue_if_unused(key, &queue);
            woken as isize
        }
        FUTEX_REQUEUE | FUTEX_CMP_REQUEUE => {
            ...
            if (uaddr2 as usize) & (core::mem::align_of::<i32>() - 1) != 0 {
                return EINVAL.as_isize();
            }

            let comparison_word = if cmd == FUTEX_CMP_REQUEUE {
                let Some(word) = try_translated_read(&mm, uaddr as *const i32) else {
                    return EFAULT.as_isize();
                };
                Some(word)
            } else {
            ...
            let Some(src_key) = mm.futex_key(VirtAddr::from(uaddr as usize), private_requested)
            else {
                return EFAULT.as_isize();
            };
            let Some(dst_key) = mm.futex_key(VirtAddr::from(uaddr2 as usize), private_requested)
            else {
                return EFAULT.as_isize();
            };
            let requeue_count = requeue_count as usize;
            let affected =
                requeue_futex_waiters(src_key, dst_key, val.max(0) as usize, requeue_count, || {
                    comparison_word.as_ref().map_or(true, |word| *word == val3)
                });
            retire_futex_wait_queue_by_key_if_unused(src_key);
            if requeue_count > 0 {
                retire_futex_wait_queue_by_key_if_unused(dst_key);
            }
            match affected {
                Ok(affected) => affected as isize,
                Err(()) => EAGAIN.as_isize(),
            }
        }
        FUTEX_WAKE_OP => {
            ...
            for (target, count) in targets {
                ...
                if (target as usize) & (core::mem::align_of::<i32>() - 1) != 0 {
                    continue;
                }
                let Some(key) = mm.futex_key(VirtAddr::from(target as usize), private_requested)
                else {
                    continue;
                };
                let queue = {
...
pub fn sys_getresgid(gid_ptr: *mut u32, egid_ptr: *mut u32, sgid_ptr: *mut u32) -> isize {
    ...
    if !gid_ptr.is_null() {
        if !try_translated_write(&mm, gid_ptr, cred.gid()) {
            return EFAULT.as_isize();
        }
    }
    if !egid_ptr.is_null() {
        if !try_translated_write(&mm, egid_ptr, cred.egid()) {
            return EFAULT.as_isize();
        }
    }
    if !sgid_ptr.is_null() {
        if !try_translated_write(&mm, sgid_ptr, cred.sgid()) {
            return EFAULT.as_isize();
        }
    }
    0
...
pub fn sys_rt_sigpending(sigset_ptr: *mut SigSet, sigsetsize: usize) -> isize {
    ...
    if !try_translated_write(&mm, sigset_ptr, pending) {
        return EFAULT.as_isize();
    }
    0
}
...
pub fn sys_unshare(flags: i32) -> isize {
    ...
    // 掩码
    const KNOWN_FLAGS: i32 = CLONE_VM
        | CLONE_FS
        | CLONE_FILES
        | CLONE_NEWNS
        | CLONE_NEWCGROUP
...
pub fn sys_riscv_hwprobe(
    ...
    if cpusetsize == 0 {
        ...
        || try_translated_read::<u8>(&mm, cpus).is_none()
        || try_translated_read::<u8>(&mm, unsafe { cpus.add(cpusetsize - 1) }).is_none()
    {
        return EFAULT.as_isize();
    }
    ...
    for index in 0..pair_count {
        let pair_ptr =
            (pairs as usize + index * core::mem::size_of::<RiscvHwprobe>()) as *mut RiscvHwprobe;
        let Some(mut pair) = try_translated_read(&mm, pair_ptr as *const RiscvHwprobe) else {
            return EFAULT.as_isize();
        };
        fill_value(&mut pair);
        if !try_translated_write(&mm, pair_ptr, pair) {
            return EFAULT.as_isize();
        }
    }
