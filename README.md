...
本项目自 2026 年 1 月创建以来，历经 5 个月、900+ commits 的迭代，已具备较为完善的管理功能与 POSIX 语义兼容性，并在多核并发的正确性保证与性能优化上取得了较好的成果。

关于**决赛的阶段性成果**：截至 8 月 12 日，内核已能在*本地*完成 **la + rv 双架构的 buildstorm 编译与 QEMU 启动验证**；受限于测评机限时，暂时尚未得到 la 的正确分，不过最后一次提交成功拿到了 rv 的正确分与部分性能分，在排行榜中的情况如下：
...
目前，决赛阶段的文档仍在撰写中：[阶段性介绍](#决赛阶段性介绍)大致描述了目前的进展，[这里](docs/rv测试结果)则展示了部分调优成果与运行日志，可结合 commit 记录与[决赛日志](docs/决赛日志.md)查看。
