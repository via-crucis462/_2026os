#!/usr/bin/env python3
"""
Wrap a RISC-V64 kernel ELF (from Rust or C) into a uImage for U-Boot.

Usage:
  python3 build_uimage_rv.py <kernel_elf> [output_uimage]

Example:
  python3 build_uimage_rv.py kernel-rv-visionfive2 kernel-rv-visionfive2.uImage
"""
...
IMAGE_NAME = b"Custom-RV-kernel\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print(f"Example: {sys.argv[0]} kernel-rv-visionfive2")
        sys.exit(1)

    elf_path = sys.argv[1]
    output = sys.argv[2] if len(sys.argv) > 2 else elf_path + ".uImage"
    build_uimage(elf_path, output)
