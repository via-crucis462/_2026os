...
## 1.概述
`ShellCore`是使用Rust语言基于[2025春夏季开源操作系统训练营 rCore 项目](https://github.com/LearningOS/rust-based-os-comp2025/blob/main/2025-spring-summary.md)的ch7逐步开发而来的，git仓库记录了自项目建立以来的commit记录。支持RISCV-V/Loongarch两个架构，兼容POSIX协议。
...
1. 启动流程：
    - 进入main前：
        - RV：参考rCore，在启动后调用run_tasks()，即idle执行流
        - Loongarch：启动前由于物理地址位数低，内核elf的高位被阶段，在物理地址启动时设置映射窗口随后打开虚拟地址，此时地址被扩展到64位，内核elf的高位地址恢复，使内核落在映射窗口中
    - 多核唤醒：RV使用了sbi_call，Loongarch参考了[2025RocketOS](https://gitlab.eduxiji.net/educg-group-36002-2710490/T202510213995926-2475)的多核唤醒流程
2. 异常处理：
    - 进入trap_handler()前参考rCore
    - 根据异常寄存器分配异常：
        - 系统调用：处理系统调用
        - 缺页异常：user_fault
...
3. AI错误与发现：
    - 在初步实现阻塞队列时，ai在进行阻塞切换前，没有释放阻塞队列的锁，导致在futex相关的测试用例中发生死锁。
    - 发现过程：我们调试的过程是先使用**日志调试**快速定位卡死的系统调用，然后再确定断点位置，使用**gdb**调试。当时卡死在了futex这个系统调用，最后调试时发现在futex唤醒时卡在了获取锁的位置。
## 5-总结与展望
- 目前我们对于Linux的理解还不够深刻，我们的内核还不够健壮，由于队员使用的是rCore ch7，导致我们的commit记录数非常庞大，不过这十分有价值，学到了很多。
- 下阶段的任务是进行代码的去耦合化以及接口的重构，增加可读性
- 完成一些自己的想法：例如：较好的调度算法，更多符合linux的设计
- 似乎还要进行驱动的开发。
## 6-参考资料
