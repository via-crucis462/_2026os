...
- **进程管理**：多级优先级队列调度，COW（写时复制），完整信号处理
...
![测评分数截图](pre-images/屏幕截图_20260630_112008.png)

关于netperf和iperf测试：

由于合并时间距离初赛结束时间剩余较短，为保证稳定性未提交到线上评测，导致没得到分数。

netprf测试均可通过，iperf测试有部分未能通过。
![iperf](pre-images/image.png)
![netprf](pre-images/image-1.png)
具体测试结果可见网盘链接里的展示视频。
视频网盘链接如下：
```text
通过网盘分享的文件：ShellCore
...
#### 线程切换
由`__switch`完成内核态的`task_ctx`切换，即切换内核态的`任务执行流`。基于这个函数和包装好的`schedule()`可以完成从某个线程的内核态任务执行流切回本核的`idle执行流`，即上次`run_tasks()`调用`__switch`的下一行代码，间接进行用户线程的状态切换和调度。

特别地，衍生出睡眠/阻塞函数（这里只讲解流程，具体可见相关代码及注释）：
- `suspend_current_and_run_next()`：先将*当前*任务的状态改为`Ready`，再`schedule()`切回idle，然后在idle中将*当前*任务放入任务池并尝试获取下一个任务。这样做是为了保证的原子性，`schedule()`保存`task_ctx`前该任务将不会被其他核拿出执行。
- `block_current_and_run_next()`：先将*当前*任务的状态改为`BlockSaving`并放入阻塞队列，再`schedule()`切回idle，然后在idle中将*当前*任务的状态改为`Blocked`。这样做是为了保证`阻塞`的原子性，`schedule()`保存好`task_ctx`前，如果有别的核希望通过`wake_up_one()`唤醒`BlockSaving`的任务，将自旋等待，直到保存完成。`BlockSaving`相当于一把自旋锁。

#### 调度策略
![alt text](pre-images/4974a989-fcbc-423c-95e9-a08c5c78ba3b.png)
...
对于调度策略为FIFO和RR，其会进入实时调度队列，即realtime；对于调度策略为其它的则会进入fair队列；对于初始进程则会进入IDLE队列；调度器由``堆``实现，其比较的索引是```优先级以及入队顺序```；调度时从deadline->realtime->fair->idle开始选择任务执行；在`run_tasks()`中，当任务数量较少时，会向TaskPool中`fetch_task()`；根据任务优先级以及调度策略分配时间片：
    ```rust
...
- `sleep_queue`：睡眠队列专用于nanosleep，在每次进入run_tasks()时更新时间戳，到达时间的则插入TaskPool
- `futex_wait_queue`：以信号量物理地址为键的BTreeMap，wake时将任务插入TaskPool
...
![alt text](pre-images/file-classes.png)
...
![alt text](pre-images/file-data-stream.png)
...
引入[pci 0.0.1](https://gitlab.com/robigalia/pci)并修改，纠正了原实现的几处错误，实现了pci设备初始化和向`PciTransport`的转换。
...
### 26-网络栈
#### 网卡与驱动
引入的是第三方依赖库的源代码作为网卡驱动，上层对其进行包装，包装代码在[io管理网络相关](#network)可见
```rust
...
//本地网卡接口，指定了假MAC地址和本地回环 IP：127.0.0.1/8
...
//外部网卡，采用QEMU默认路由网关10.0.2.2和默认分发给虚拟机的ip地址10.0.2.15
...
```
tcp和udp都实现了file特性，可以作为文件进行读写，读写的过程中调用Net_poll。
net_poll通过smoltcp提供的函数处理硬件网卡中的数据。
根据对应的端口将数据接收或者发送到对应handle的缓冲区里。
```rust
pub fn net_poll() {
    while budget > 0 {
        budget -= 1;
...
```
udp/tcp的读写便是依赖环形缓冲区与下层关联，如部分tcp代码所示。
```rust
if socket.can_recv() {
...
if socket.can_send() {
    //从内核把数据拷贝到了smoltcp的tx_buffer 内存
    let write_len = socket.send_slice(&temp_buf).unwrap_or(0);
    if write_len > 0 {
        drop(sockets); 
        crate::net::net_poll();//通过net_poll对底层网卡进行驱动，发送数据
        return write_len; 
    }
}
...
在回收的时候，为了避免直接关闭而导致tcp结束的四次握手没法正常完成的情况，先是调用close，而后在net_poll里检查状态以及全局队列里不再有该handle的情况下完成清除，以tcp为例：
```rust
...
```
关于raw socket，支持ping命令以及相关解析匹配。
unix socket提供本地进程间通信，不过这两个socket还未经过完整的测试，且存在较旧的代码未被重构。
在日后完善。

StandardNetlinkSocket：
支持ip addr给系统添加ip地址以及符合linux内核标准的LinkReplyPacket。
```rust
...
在全局初始化的时候设置了读写队列，但是当前内核关中断，导致阻塞后会有陷入挂起接收不到中断信号没法唤醒的状况。
于是改为轮询，但保留了队列，以及net_poll的唤醒机制，在后续完成内核中断后可进行优化。

对于定时超时的处理，由于上述所说，在当前关中断下选择了在read和write的时候手动驱动当前定时器进行检查。
```rust
 crate::timer::check_timer_cooperative();
            ...
            drop(task_inner);
```
crate::timer::check_timer_cooperative()函数与时钟中断处理的处理流程等同不再赘述，这里包装成函数便于重复使用。
明显可见会有较大性能损耗，可在后续优化。
## 3-调试过程
...
- `StandardNetlinkSocket` `rawsocke`代码的基础生成；
- 收发报文格式的定义，如`IfAddressMsg`；
- 部分简单基础系统调用，如`sys_sysinfo`获得基础信息之类的调用；
- `epoll`机制的相关早期实现。
...
4. base：[2025春夏季开源操作系统训练营 rCore-Tutorial-v3 ch7](https://github.com/rcore-os/rCore-Tutorial-v3/tree/ch7)
5. Linux源码
